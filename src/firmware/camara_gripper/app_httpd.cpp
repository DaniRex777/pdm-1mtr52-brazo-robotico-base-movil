...
static esp_err_t gripper_handler(httpd_req_t *req) {
  char query[64];
  char action[16];
  ...
  if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
    if (httpd_query_key_value(query, "action", action, sizeof(action)) == ESP_OK) {
      ...
      if (strcmp(action, "open") == 0) {
        ...
        httpd_resp_send(req, "open ok", HTTPD_RESP_USE_STRLEN);
        return ESP_OK;
      }
      ...
      if (strcmp(action, "close") == 0) {
        ...
        httpd_resp_send(req, "close ok", HTTPD_RESP_USE_STRLEN);
        return ESP_OK;
      }
    }
  }

  httpd_resp_send(req, "accion invalida", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &gripper_uri);
}
